...
def writefile(program, file_ext, function, unit):
    ...
    string = "./" + function + " " + string + " > Simulation/coverage_matrices/" + program + "_" + unit + ".txt"
...
if program == "RSEM":
    ...
    writefile("coverage", working_dir+"/"+results+"/*.sim.isoforms.results", "rsem-generate-data-matrix4", "Counts")
    print working_dir+"/"+results+"/*.sim.isoforms.results"
#    writefile("ground_truth", "Simulation/data/simulated/*.isoforms.results", "rsem-generate-data-matrix5", "TPM")
 #   writefile("ground_truth", "Simulation/data/simulated/*.isoforms.results", "rsem-generate-data-matrix6", "FPKM")

else:
    writefile(program, working_dir+"/"+results+"/*.sim.isoforms.results", "rsem-generate-data-matrix5", "TPM")
